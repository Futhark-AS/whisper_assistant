...
## 2. Completeness Gaps
- Current Python config validation depth is not fully represented. The existing app performs strict hotkey parsing, aggregates multiple config errors before startup, and supports explicit `TRANSCRIPTION_OUTPUT=none` semantics (`src/quedo/env.py`). The architecture’s parity list is too high-level and risks dropping these guardrails.
- Python has a concrete stream recovery path: retry once on `stream_open_error` with user feedback before hard failure (`src/quedo/main.py`). The architecture describes recovery conceptually but does not clearly preserve this behavior.
- Migration coverage for history/media is incomplete. Current history workflows rely on `recording.flac` with `.wav` fallback and CLI playback/transcribe flows (`src/quedo/main.py`, `src/quedo/cli.py`). The migration section should explicitly preserve both file variants and behaviors.
- Chunking diagnostics are under-specified. The current transcriber logs contextual windows around chunk boundaries to debug cut-word issues (`src/quedo/packages/transcriber/main.py`). The architecture mentions chunking, but not this diagnostic capability.
