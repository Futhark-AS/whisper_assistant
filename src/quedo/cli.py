...
from quedo import __version__
from quedo.env import VALID_WHISPER_MODELS, ConfigErrors, read_env
from quedo.packages.transcriber import Transcriber
from quedo.paths import (
    get_config_file,
    get_history_dir,
    get_log_dir,
    get_pid_file,
)
from quedo.permissions import (
    SYM_FAIL,
    SYM_OK,
    SYM_WARN,
    check_all,
...
@click.version_option(
    version=__version__,
    prog_name="quedo",
    message="%(prog)s %(version)s\n\nTo upgrade: uv tool install quedo --force --from git+https://github.com/Futhark-AS/quedo.git",
)
def cli() -> None:
    """Quedo CLI."""
    pass
...
def init() -> None:
    ...
    click.echo()
    click.secho("🎙️  Quedo Setup", fg="cyan", bold=True)
    click.echo("=" * 40)
    click.echo()
    ...
    if not api_key or api_key == "your_api_key":
        click.secho("Invalid API key. Run 'quedo init' again.", fg="red")
        sys.exit(1)
...
"""
    ...
    try:
        ...
        click.echo("   Run 'quedo config edit' to fix.")
        return
    ...
    if failed:
        ...
        if failed:
            click.echo()
            click.echo("  You can fix later with: quedo doctor --fix")

    click.echo()

    # Offer to start daemon
    if click.confirm("Start quedo now?", default=True):
        click.echo()
        _start_daemon()
        click.echo()
        ...
        click.echo()
        click.echo("Run 'quedo start' when ready.")
...
def _print_hotkey_info() -> None:
    ...
    try:
        ...
        click.echo()
        click.echo("  Change hotkeys: quedo config edit")
    except Exception:
        pass
...
def _start_daemon() -> bool:
    ...
    if is_running():
        pid = get_pid()
        click.echo(f"Quedo is already running (PID: {pid})")
        return False
    ...
    if failed:
        ...
        click.echo("   Fix with: quedo doctor --fix")
        click.echo()
    ...
    try:
        ...
        if exit_code is not None:
            click.echo(
                f"Quedo crashed during startup (exit code: {exit_code})",
                err=True,
            )
            # Show stderr output
        ...
        click.echo(f"Quedo started (PID: {process.pid})")
        _print_hotkey_info()
        return True
    except Exception as e:
        click.echo(f"Failed to start Quedo: {e}", err=True)
        return False
...
def _stop_daemon() -> bool:
    ...
    if not is_running():
        click.echo("Quedo is not running")
        return False
    ...
    try:
        ...
        click.echo(f"Failed to stop Quedo: {e}", err=True)
        return False


@cli.command()
def start() -> None:
    """Start the Quedo daemon."""
...
@cli.command()
def status() -> None:
    """Check the status of Quedo."""
    if is_running():
        pid = get_pid()
        click.echo(f"Quedo is running (PID: {pid})")
    else:
        click.echo("Quedo is not running")
...
def doctor(fix: bool) -> None:
    ...
    try:
        ...
        for err in e.errors:
            click.echo(f"     {err}")
        click.echo(f"     File: {config_file}")
        issues.append("Fix config: quedo config edit")
    ...
    if is_running():
        ...
        issues.append("Start daemon: quedo start")
    ...
    if fix and any(not perms[n] for n in perms):
        ...
        click.echo("  Add your terminal app, then re-run: quedo doctor")
